...
public class TargetCam extends Subsystem
{
	...
	public static void startVision()
	{
		...
		try
		{
			// launch the GRIP process - don't wait - it doesn't appear to
			// return, not sure why though
			pb.redirectErrorStream(true);
			pb.inheritIO().start();
	...
	public static void stopVision()
	{
		...
		try
		{
			...
			pb.redirectErrorStream(true);
			Process process = pb.inheritIO().start();
			result = process.waitFor(); // wait for the ssh to return, but it
										// might return saying it couldn't
										// connect
			if (result == 0)
			{
