...
//		SmartDashboard.putNumber("X getEncoderSetpoint", encoderPosition);
...
//			SmartDashboard.putNumber("X encoderPosition", encoderPosition);
//			SmartDashboard.putNumber("getLeftDistance", Robot.chassis.getLeftDistance());
	 ...
	 * the target
	 */
	private boolean robotAlignedToTarget()
	{
		// Determine if the robot is lined to the target
		if ((Math.abs(targetShooterPositionX() - Robot.targets.getTargetPixel_X()) <= Constants.IMAGE_ON_TARGET_X) &&
			(Math.abs(targetShooterPositionY() - Robot.targets.getTargetPixel_Y()) <= Constants.IMAGE_ON_TARGET_Y))
		{
			return true;
		} 
