 ...
 */
public class HVAGyro extends GyroBase implements Gyro, PIDSource, LiveWindowSendable
{
	 ...
	 *            used on on-board channels 0-1.
	 * @param center
	 *            Preset uncalibrated value to use as the accumulator center
	 *            value.
	 * @param offset
	 *            Preset uncalibrated value to use as the gyro offset.
	 */
	public HVAGyro(int channel, int center, double offset)
	{
	 ...
	 *            Preset uncalibrated value to use as the gyro offset.
	 */
	public HVAGyro(AnalogInput channel, int center, double offset)
	{
		m_analog = channel;
		if (m_analog == null)
		{
	...
	public double getAngle()
	{
		...
		else
		{
			...
			double scaledValue = value * 1e-9 * m_analog.getLSBWeight() * (1 << m_analog.getAverageBits()) / (AnalogInput.getGlobalSampleRate() * m_voltsPerDegreePerSecond);
	...
	public double getRate()
	{
		...
		{
			return (m_analog.getAverageValue() - (m_center + m_offset)) * 1e-9 * m_analog.getLSBWeight() / ((1 << m_analog.getOversampleBits()) * m_voltsPerDegreePerSecond);
		}
	}

	/**
	 * Return the gyro offset value set during calibration to use as a future
	 * preset
	 *
	...
	/**
	 * Return the gyro center value set during calibration to use as a future
	 * preset
	 *
