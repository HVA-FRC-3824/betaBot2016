...
public class USBCamera {
	...
	public synchronized void updateSettings() {
		...
		if (m_exposure == "auto")
			...
			if (m_exposureValue != -1) {
				...
				long val = (long) m_exposureValue; //minv + (long)(((double)(maxv - minv)) * (((double)m_exposureValue) / 100.0));
	...
	public synchronized int getBrightness() {
		long val = NIVision.IMAQdxGetAttributeI64(m_id, ATTR_BR_VALUE);
		long minv = NIVision.IMAQdxGetAttributeMinimumI64(m_id, ATTR_BR_VALUE);
		long maxv = NIVision.IMAQdxGetAttributeMaximumI64(m_id, ATTR_BR_VALUE);
		int brightness = (int) ((100.0 * ((double)(val - minv) / (double)(maxv - minv))) + 0.5) ;
		return brightness;
		//return m_brightness;
	}
	
	public synchronized String getExposureMode()
	{
		return 	NIVision.IMAQdxGetAttributeString(m_id, ATTR_EX_MODE);
	}
	
	public synchronized String getExposure()
	{
		String exposure;
//		String exposure = NIVision.IMAQdxGetAttributeString(m_id, ATTR_EX_MODE);
//		if(exposure.compareTo("Manual") == 0)
//		{
			long val = NIVision.IMAQdxGetAttributeI64(m_id, ATTR_EX_VALUE);
			long minv = NIVision.IMAQdxGetAttributeMinimumI64(m_id, ATTR_EX_VALUE);
			long maxv = NIVision.IMAQdxGetAttributeMaximumI64(m_id, ATTR_EX_VALUE);
			// exposure = String.valueOf(100.0 * ((double)(val - minv) / (double)(maxv - minv))) ;
			exposure = String.valueOf(val);
//		}
		
		return exposure;
	}
	
	public synchronized long getExposureMin()
	{
		return NIVision.IMAQdxGetAttributeMinimumI64(m_id, ATTR_EX_VALUE);
	}
	
	public synchronized long getExposureMax()
	{
		return NIVision.IMAQdxGetAttributeMaximumI64(m_id, ATTR_EX_VALUE);
	}
